...
const whatsappCountries = [
    ...
    { name: 'Canada', code: '+1', price: 187, countryId: 36, flag: '🇨🇦' },
...
function extendWhatsappCountriesFromFile(catalog) {
    ...
    try {
        ...
        const parsedCountryIds = new Set(parsed.countries.map((country) => String(country?.countryId ?? '').trim()).filter(Boolean));
        const seededExistingCountries = existingCountries.map((country) => {
            const countryIdKey = String(country?.countryId ?? '').trim();
            if (!parsedCountryIds.has(countryIdKey)) return country;
            const existingOtpQuality = country?.otpQuality && typeof country.otpQuality === 'object'
                ? country.otpQuality
                : {};
            return {
                ...country,
                otpQuality: {
                    ...lowOtpSeed,
                    ...existingOtpQuality,
                    label: existingOtpQuality.label || existingOtpQuality.defaultLabel || 'Low OTP',
                    defaultLabel: 'Low OTP',
                    minimumCompletedOrders: 3
                }
            };
        });
        const seededExistingCountryIds = new Set(seededExistingCountries.map((country) => String(country?.countryId ?? '').trim()).filter(Boolean));
        const lowOtpCountries = parsed.countries
            .filter((country) => !seededExistingCountryIds.has(String(country?.countryId ?? '').trim()))
            .map((country) => ({
                ...country,
                code: country.code || (normalizeCountryNameForFileLookup(country.name) === 'australia' ? '+61' : ''),
                otpQuality: { ...lowOtpSeed }
            }));
        if (!lowOtpCountries.length) {
            whatsappService.countries = seededExistingCountries;
            return;
        }
        const usaIndex = seededExistingCountries.findIndex((country) => String(country?.name || '').trim().toLowerCase() === 'usa' && Number(country?.countryId) === 187);
        whatsappService.countries = usaIndex >= 0
            ? [
                ...seededExistingCountries.slice(0, usaIndex + 1),
                ...lowOtpCountries,
                ...seededExistingCountries.slice(usaIndex + 1)
            ]
            : [...seededExistingCountries, ...lowOtpCountries];
    } catch {
    }
}
