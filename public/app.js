...
async function loadAvailableServices() {
    ...
    try {
        ...
        const extraServices = parseExtraServicesFromText(rawText);
        state.availableServices = [...getDefaultServiceCatalog(), ...extraServices];
        renderServiceGrid();
    } catch {
    }
}
...
function renderServiceSvg(serviceType) {
    switch (serviceType) {
        ...
        case 'imo':
            return '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="12" fill="#00A6FF"></circle><text x="12" y="14.6" font-size="6.8" font-family="Inter, Arial, sans-serif" text-anchor="middle" font-weight="800" fill="#ffffff">imo</text></svg>';
        case 'tinder':
        case 'twitter':
        case 'amazon':
        case 'alibaba':
...
function getOrderLifecycleStatus(order) {
    ...
    if (['pending', 'active', 'completed', 'expired', 'cancelled'].includes(rawStatus)) return rawStatus;
    return 'pending';
}
...
function updateActivationSummaryLine() {
    ...
    if (state.activationFilter === 'waiting') {
        summaryLine.textContent = `Active order table ${waitingOrders.length} pcs / Value ${formatMoneyPrecise(waitingAmount)}`;
        syncWaitingOrdersVisibility();
        return;
    }
...
function syncWaitingOrdersVisibility() {
    ...
    ordersList.classList.toggle('hidden', state.historyView !== 'activations' || !state.currentUser);
}
...
function setHistoryView(view, options = {}) {
    ...
    if (title) {
        title.textContent = normalizedView === 'payments' ? 'Payment Details' : 'Phone Activation History';
    }
    if (titleCopy) {
        titleCopy.textContent = normalizedView === 'payments'
            ? 'View pending, success, and cancel payment requests from your account.'
            : 'Track waiting numbers instantly and switch to payment detail whenever you need it.';
    }
...
function getCountryOtpQualityMeta(country) {
    const rawQuality = country?.otpQuality && typeof country.otpQuality === 'object' ? country.otpQuality : {};
    const totalOrders = Number(rawQuality.totalOrders || 0);
    const successfulOrders = Number(rawQuality.successfulOrders || 0);
    const completedOrders = Number(rawQuality.completedOrders || successfulOrders || 0);
    const successRate = Number.isFinite(Number(rawQuality.successRate)) ? Number(rawQuality.successRate) : 0;
    const minimumCompletedOrders = Number(rawQuality.minimumCompletedOrders || 20);
    const hasEnoughData = completedOrders >= minimumCompletedOrders && Boolean(rawQuality.hasEnoughData);
    const defaultLabel = String(rawQuality.defaultLabel || rawQuality.label || 'Good OTP');
    const label = hasEnoughData ? String(rawQuality.label || defaultLabel) : defaultLabel;
    const tone = label === 'Average OTP'
        ? 'average'
        : label === 'Low OTP'
            ? 'low'
            : 'good';
...
function renderCountryOtpBadge(country) {
    const otpQuality = getCountryOtpQualityMeta(country);
    return `
        <span class="country-otp-badge country-otp-badge--${escapeAttr(otpQuality.tone)}">
            <span class="country-otp-badge-dot" aria-hidden="true"></span>
            <span>${escapeHtml(otpQuality.label)}</span>
        </span>
    `;
}

function sortCountriesByOtpQuality(countries, serviceType = '') {
    const list = Array.isArray(countries) ? [...countries] : [];
    if (String(serviceType || '').trim().toLowerCase() === 'whatsapp') {
    ...
    const hasRankableCountries = list.some((country) => getCountryOtpQualityMeta(country).hasEnoughData);
    return list.sort((left, right) => {
        const leftMeta = getCountryOtpQualityMeta(left);
        const rightMeta = getCountryOtpQualityMeta(right);
        const leftCatalogOrder = Number(left?.catalogOrder ?? Number.MAX_SAFE_INTEGER);
        const rightCatalogOrder = Number(right?.catalogOrder ?? Number.MAX_SAFE_INTEGER);
        if (!hasRankableCountries) return leftCatalogOrder - rightCatalogOrder;
        if (leftMeta.hasEnoughData !== rightMeta.hasEnoughData) {
            return leftMeta.hasEnoughData ? -1 : 1;
        }
        if (leftMeta.hasEnoughData && rightMeta.hasEnoughData) {
            if (leftMeta.successRate !== rightMeta.successRate) return rightMeta.successRate - leftMeta.successRate;
            if (leftMeta.successfulOrders !== rightMeta.successfulOrders) return rightMeta.successfulOrders - leftMeta.successfulOrders;
            if (leftMeta.totalOrders !== rightMeta.totalOrders) return rightMeta.totalOrders - leftMeta.totalOrders;
        }
        return leftCatalogOrder - rightCatalogOrder;
    });
}

function logCountryOtpQualityDebug(countries) {
    (countries || []).forEach((country) => {
        const otpQuality = getCountryOtpQualityMeta(country);
        console.log(
            `[Country OTP] ${String(country?.name || 'Unknown')} | total orders: ${otpQuality.totalOrders} | successful OTP: ${otpQuality.successfulOrders} | success rate: ${(otpQuality.successRate * 100).toFixed(1)}%`
        );
    });
}
...
function renderCountries() {
    ...
    filtered = sortCountriesByOtpQuality(filtered, state.currentService);
    if (!filtered.length) {
...
async function loadCountries() {
    try {
        ...
        logCountryOtpQualityDebug(state.allCountries);
        renderCountries();
    } catch (err) {
        state.allCountries = [];
        renderCountries();
...
function mergeOrdersWithLocalState(fetchedOrders) {
    const normalizedFetchedOrders = Array.isArray(fetchedOrders) ? [...fetchedOrders] : [];
    ...
    return [...localPendingOrders, ...normalizedFetchedOrders].sort((left, right) => new Date(right.created_at) - new Date(left.created_at));
}
...
function renderActiveOrders(orders) {
    ...
    container.innerHTML = filteredOrders.map((order) => {
        ...
        const statusNote = isOtpReady
            ? 'OTP received. Tap Complete to close this order.'
            : isWaitingOrder(order)
                ? 'Waiting for an incoming SMS.'
                : lifecycleStatus === 'expired'
                    ? 'Expired and refunded to wallet.'
                    : lifecycleStatus === 'cancelled'
                        ? 'Cancelled and refunded to wallet.'
                        : 'Activation completed successfully.';
        const primaryStatus = getSmsOrderPrimaryStatus(order, lifecycleStatus);
        const statusValue = lifecycleStatus === 'pending'
            ? 'Waiting'
        ...
        const contentPanel = isOtpReady
            ? `
                    <div class="sms-order-otp-box">
                        <div class="sms-order-otp-label">OTP Code</div>
            ...
            : isVisibleInWaiting
                ...
                `
                : `
                    <div class="sms-order-otp-box sms-order-otp-box--muted">
                        <div class="sms-order-otp-label">${escapeHtml(primaryStatus)}</div>
                        <div class="sms-order-live-text">${escapeHtml(statusNote)}</div>
                    </div>
                `;
        const actionButtons = isOtpReady
            ? `
                <button class="sms-order-btn complete" data-action="complete-order" data-order-id="${escapeAttr(order.id)}">Complete</button>
            `
            : lifecycleStatus === 'pending'
                ...
                `
                : '';
        const footerNotes = lifecycleStatus === 'pending'
            ? [
                canCancel
                    ? '<span>Cancel available now</span>'
                    : `<span>Cancel available in <span data-cancel-unlock-id="${escapeAttr(order.id)}">${escapeHtml(getCancelUnlockText(order))}</span></span>`
            ]
            : ['completed', 'cancelled', 'expired'].includes(lifecycleStatus)
                ? [`<span>${escapeHtml(statusNote)}</span>`]
                : [];
        const footerMarkup = footerNotes.length
            ? `<div class="sms-order-foot">${footerNotes.map((note) => `<div class="sms-order-secondary-note">${note}</div>`).join('')}</div>`
            : '';
        const actionAreaMarkup = lifecycleStatus === 'pending'
            ? `<div class="sms-order-cancel-panel ${canCancel ? 'is-ready' : 'is-locked'}">${actionButtons ? `<div class="sms-order-actions">${actionButtons}</div>` : ''}${footerMarkup}</div>`
            : `${actionButtons ? `<div class="sms-order-actions">${actionButtons}</div>` : ''}${footerMarkup}`;
        return `
            <article class="sms-order-card ${isOtpReady ? 'sms-order-card--ready' : lifecycleStatus === 'pending' ? 'sms-order-card--waiting' : 'sms-order-card--closed'}">
                ...
                <div class="sms-order-info-row">
                    <span>Price: <strong>${escapeHtml(formatMoneyPrecise(order.price))}</strong></span>
                    <span class="sms-order-info-divider" aria-hidden="true"></span>
                    <span>Expire: ${expireValueMarkup}</span>
                    <span class="sms-order-info-divider" aria-hidden="true"></span>
                    <span class="sms-order-info-status">Status: ${statusDetailMarkup}</span>
                </div>
                ${contentPanel}
                ${actionAreaMarkup}
            </article>
...
async function loadAdminData() {
    ...
    try {
        ...
        const [orders, paymentRequests, ledgerTransactions, pendingTransactions, users, balanceAdjustments] = await Promise.all([
...
async function refreshUserInfo() {
    ...
    try {
        ...
        if (state.activeOrder?.id != null) {
            state.activeOrder = orders.find((entry) => String(entry.id) === String(state.activeOrder.id)) || state.activeOrder;
        }
