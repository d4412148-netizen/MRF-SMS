...
const serviceBrandLogoUrls = {
    imo: 'https://cdn.simpleicons.org/imo/2563EB',
    tinder: 'https://cdn.simpleicons.org/tinder/EF4444',
    twitter: 'https://cdn.simpleicons.org/x/111827',
    amazon: 'https://cdn.simpleicons.org/amazon/F59E0B',
    alibaba: 'https://cdn.simpleicons.org/alibaba/EA580C',
    careem: 'https://cdn.simpleicons.org/careem/16A34A',
    spotify: 'https://cdn.simpleicons.org/spotify/16A34A',
    openai: 'https://cdn.simpleicons.org/openai/111827',
    paypal: 'https://cdn.simpleicons.org/paypal/0369A1',
    aliexpress: 'https://cdn.simpleicons.org/aliexpress/EA580C',
    wechat: 'https://cdn.simpleicons.org/wechat/65A30D',
    viber: 'https://cdn.simpleicons.org/viber/7C3AED',
    uber: 'https://cdn.simpleicons.org/uber/111827',
    microsoft: 'https://cdn.simpleicons.org/microsoft/0EA5E9',
    signal: 'https://cdn.simpleicons.org/signal/3B82F6',
    easypay: 'https://cdn.simpleicons.org/easypaisa/CA8A04'
};

const BUILT_IN_SERVICE_ORDER = Object.keys(serviceMeta);
const TERMS_ACCEPTED_STORAGE_KEY = 'mrf-terms-accepted-v1';
const EXTRA_SERVICE_ICON_SLUGS = {
    telegram: 'telegram',
    discord: 'discord',
    linkedin: 'linkedin',
    youtube: 'youtube',
...
function getGeneratedServiceLogoUrl(label) {
    const lookupKey = normalizeServiceLookup(label).replace(/\s+/g, '');
    const iconSlug = EXTRA_SERVICE_ICON_SLUGS[lookupKey];
    return iconSlug ? `https://cdn.simpleicons.org/${iconSlug}` : '';
}
...
function createGeneratedServiceMeta(serviceType, label, options = {}) {
    ...
    const comingSoon = Boolean(options.comingSoon);
    const generatedMeta = {
        label: displayLabel,
        shortLabel,
        ...
        wrapperTone: 'bg-emerald-500/10 ring-1 ring-emerald-400/20',
        logoUrl: getGeneratedServiceLogoUrl(displayLabel),
        generated: true,
        comingSoon
    };
...
function parseExtraServicesFromText(rawText) {
    ...
    for (let index = 0; index < tokens.length - 1; index += 1) {
        ...
        createGeneratedServiceMeta(serviceType, label, { comingSoon: true });
        extras.push({
            serviceType,
            label,
            isBuiltIn: false,
...
function getServiceMeta(serviceType) {
    ...
    return createGeneratedServiceMeta(serviceType, serviceType, { comingSoon: true });
}
...
function renderFallbackServiceGlyph(serviceType) {
    ...
    const fallbackInner = meta.iconClass
        ? `<i class="${escapeAttr(meta.iconClass)} ${escapeAttr(meta.iconTone || 'text-slate-700')} text-[0.92em]"></i>`
        : `<span class="text-[0.68em] font-black uppercase tracking-[0.18em] text-emerald-700">${fallbackText}</span>`;
    return `<span class="inline-flex h-full w-full items-center justify-center rounded-[7px] ${meta.wrapperTone || ''}">${fallbackInner}</span>`;
}

function renderServiceBrandImageGlyph(serviceType) {
    const meta = getServiceMeta(serviceType);
    const logoUrl = meta.logoUrl || serviceBrandLogoUrls[serviceType];
    if (!logoUrl) return '';
    return `
        <span class="relative inline-flex h-full w-full items-center justify-center rounded-[7px] bg-white/95 p-[2px]">
            <img src="${logoUrl}" alt="${escapeAttr(meta.label)} logo" loading="lazy" referrerpolicy="no-referrer" class="h-full w-full object-contain rounded-[6px]" onerror="this.classList.add('hidden');this.nextElementSibling.classList.remove('hidden');">
            <span class="hidden h-full w-full">${renderFallbackServiceGlyph(serviceType)}</span>
        </span>
    `;
}
...
function renderServiceLogo(serviceType, size = 'md') {
    ...
    const customMarkup = renderServiceSvg(serviceType);
    const imageMarkup = renderServiceBrandImageGlyph(serviceType);
    const finalMarkup = customMarkup || imageMarkup || renderFallbackServiceGlyph(serviceType);
...
function closeModal(id) {
    ...
    if (!document.querySelector('.app-modal:not(.hidden):not(#processing-modal)')) {
...
function renderCountries() {
    ...
    const isCatalogOnlyService = !BUILT_IN_SERVICE_ORDER.includes(state.currentService);
    ...
    if (!filtered.length) {
        const title = isCatalogOnlyService
            ? `${currentMeta.label} numbers are coming soon`
            : 'No countries found';
        const message = isCatalogOnlyService
            ? `${currentMeta.label} has been added to the service list. Live country inventory will appear here as soon as this platform is connected.`
...
async function loadCountries() {
    if (!BUILT_IN_SERVICE_ORDER.includes(state.currentService)) {
        state.allCountries = [];
        renderCountries();
        return;
    }
    try {
        ...
        state.allCountries = Array.isArray(countries) ? countries : [];
        state.allCountries = state.allCountries.sort((left, right) => {
            const leftPrice = Number(left?.price ?? Number.POSITIVE_INFINITY);
            const rightPrice = Number(right?.price ?? Number.POSITIVE_INFINITY);
            if (leftPrice !== rightPrice) return leftPrice - rightPrice;
            return String(left?.name || '').localeCompare(String(right?.name || ''));
...
function renderActiveOrders(orders) {
    ...
    container.innerHTML = filteredOrders.map((order) => {
        ...
        const waitingDescription = isOtpReady
            ? 'The OTP has arrived. Use Complete to close this order.'
...
function updateProcessingModalState() {
    const modal = qs('processing-modal');
    if (!modal) return;
    const stickyHeader = document.querySelector('.mx-auto > .sticky');
    const topAlert = qs('payment-top-alert');
    const topAlertHeight = topAlert && !topAlert.classList.contains('hidden')
        ? Math.ceil(topAlert.getBoundingClientRect().height || 0)
        : 0;
    const stickyHeaderHeight = stickyHeader ? Math.ceil(stickyHeader.getBoundingClientRect().height || 0) : 0;
    modal.style.paddingTop = `${Math.max(86, topAlertHeight + stickyHeaderHeight + 12)}px`;
    const activeRequests = [...state.purchaseRequests.values()];
    const requestCount = activeRequests.length;
    const summary = qs('processing-active-count');
    const details = qs('processing-active-details');

    if (summary) {
        summary.textContent = requestCount === 1
            ? '1 purchase request is running'
            : `${requestCount} purchase requests are running`;
    }

    if (details) {
        const detailText = activeRequests
            .slice(0, 2)
            .map((entry) => `${getServiceMeta(entry.serviceType).label} • ${entry.countryName}`)
            .join(' | ');
        details.textContent = detailText || 'Please wait while we search available numbers.';
    }
